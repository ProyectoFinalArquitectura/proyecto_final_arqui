	...
	return (
		<main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-black px-4 py-8 text-white md:px-6">
			...
			<div className="relative w-full max-w-[30rem] rounded-2xl border border-white/10 bg-[#111111]/95 p-6 shadow-2xl transition-all duration-300 md:p-8">
				...
				<div className="absolute right-4 top-4 h-7 w-7 rounded-full bg-[var(--color-primary)]/95 text-center text-sm font-bold leading-7 text-black">
					x
				</div>

				<div className="relative">
					<div className="mb-6 flex items-center gap-3">
						<Image src="/assets/logo/logo_vinfinite.png" alt="Vinfinite" width={42} height={42} className="rounded-lg" />
						<div>
							<p className="text-base font-semibold tracking-wide">vinfinite</p>
							<p className="text-[0.75rem] text-white/65">Welcome to your event journey</p>
						</div>
					</div>
					...
					<form className="mt-6 space-y-4" onSubmit={onSubmit} noValidate>
						...
						<label className="block">
							...
							/>
							{formErrors.password && (
								<p className="mt-2 text-[0.75rem] text-[var(--color-primary)]">{formErrors.password}</p>
							)}
						</label>
